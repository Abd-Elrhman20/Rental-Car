const express = require("express");
const licenseMiddelware = require("../middlewares/licenseMiddelware");
const router = express.Router();
const User = require("../models/userModel")
// //////////////////////
const vision = require('@google-cloud/vision');

const CREDENTIALS = JSON.parse(JSON.stringify({
    "type": "service_account",
    "project_id": "rich-howl-379813",
    "private_key_id": "70aa4dbf50723c678a96764699a917c7f10652a7",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "visio-445@rich-howl-379813.iam.gserviceaccount.com",
    "client_id": "109535144642653371753",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/visio-445%40rich-howl-379813.iam.gserviceaccount.com",
    "universe_domain": "googleapis.com"
}));

const CONFIG = {
    credentials: {
        private_key: CREDENTIALS.private_key,
        client_email: CREDENTIALS.client_email
    }
};

const client = new vision.ImageAnnotatorClient(CONFIG);
// //////////////////////

router.post("/login", async (req, res) => {

    const { email, password } = req.body

    try {
        const user = await User.findOne({ email, password })
        if (user) {
            res.send(user)
        }
        else {
            return res.status(400).json(error);
        }
    } catch (error) {
        return res.status(400).json(error);
    }

});

router.post("/register", async (req, res) => {

    try {
        const newuser = new User(req.body)
        await newuser.save()
        res.send('User registered successfully')
    } catch (error) {
        return res.status(400).json(error);
    }

});

///////////////////////////////////////////////////////////// (S)(upload img)

router.get('/getLicense/', async (req, res) => {
    const allPhotos = await User.find().sort({ createdAt: "descending" });
    res.send(allPhotos);
});

router.get('/getLicense/:email', async (req, res) => {
    const allPhotos = await User.find({ email: req.params.email }).sort({ createdAt: "descending" });
    res.send(allPhotos);
});

router.post('/saveLicense/:email', licenseMiddelware.single("photo"), (req, res) => {
    const photo = req.file.filename;
    // const photo = req.file.path;
    console.log(photo);
    console.log(req.params.email);
    User.findOneAndUpdate({ email: req.params.email }, { photo })
        .then((data) => {
            console.log("Uploaded Successfully ..... ");
            console.log(data);
            // console.log(photo);
            // send data and photo to the front
            res.send({ data, photo });

        })
        .catch((err) => console.log(err))

});
///////////////////////////////////////////////////////////// (E)(upload img)

///////////////////////////////////////////////////////////// (S)(upload {detections} to current user)
// do a route to add the license to the current user
router.post('/google/:email', (req, res) => {
    console.log(req.body.src);
    const imgpath = `public\\driveLicense\\${req.body.src}`
    // console.log(req.params.email);

    client
        .textDetection(imgpath)
        .then(results => {
            const [result] = results
            const detections = result.fullTextAnnotation.text.split('\n')[15].slice(15);
            console.log(detections);
            User.findOneAndUpdate({ email: req.params.email }, { licenseIMG: detections })
                .then((data) => {
                    console.log("Uploaded Successfully ..... ");
                    console.log(data);
                    res.send({ data, detections });
                })
        })
        .catch(err => {
            console.error('ERROR:', err);
        });

});
///////////////////////////////////////////////////////////// (E)(upload {detections} to current user)



// do a to add property isAllowd to the current user
router.post('/checkLicense/:email', (req, res) => {
    const { isAllowed } = req.body;
    console.log(isAllowed);
    // console.log(req.params.email);
    User.findOneAndUpdate({ email: req.params.email }, { isAllowed : isAllowed })
        .then((data) => {
            res.send({ isAllowed });
        })
});



module.exports = router

